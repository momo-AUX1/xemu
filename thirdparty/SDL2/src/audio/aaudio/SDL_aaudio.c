...
static int aaudio_OpenDevice(_THIS, const char *devname)
{
    ...
    if (iscapture) {
        if (captureDevice && captureDevice != this) {
            SDL_Log("AAudio: capture device already open, overwriting");
        }
    } else {
        if (audioDevice && audioDevice != this) {
            SDL_Log("AAudio: output device already open, overwriting");
        }
    }
...
static void aaudio_CloseDevice(_THIS)
{
    ...
    if (this->iscapture) {
        if (captureDevice == this) {
            captureDevice = NULL;
        } else {
            SDL_Log("AAudio: capture close mismatch");
        }
    } else {
        if (audioDevice == this) {
            audioDevice = NULL;
        } else {
            SDL_Log("AAudio: output close mismatch");
        }
    }
